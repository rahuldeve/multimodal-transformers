...
class AV_Encoder(nn.Module):
    def __init__(self, hidden_size, beta_shift, dropout_prob):
        super(AV_Encoder, self).__init__()
        print(
...
class AVPrompt_PositionBertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        embeddings = modal_input + token_type_embeddings
...
class AVPrompt_MiniLM(BertModel):
    def __init__(self, config, multimodal_config):
        ...
        self.fuse = True
        self.prompt =  AVPrompt_PositionBertEmbeddings(config)
    ...
    def forward(
        ...
        if self.fuse:
            ...
            audio_embs = self.prompt(
                input_ids=input_ids, 
                modal_input=audio_embs,
                position_ids=position_ids,
                token_type_ids=token_type_ids,
                inputs_embeds=inputs_embeds,
                past_key_values_length=past_key_values_length,
            ...
            video_embs = self.prompt(
                input_ids=input_ids, 
                modal_input=video_embs,
                position_ids=position_ids,
                token_type_ids=token_type_ids,
                inputs_embeds=inputs_embeds,
                past_key_values_length=past_key_values_length,
...
class AVPrompt_MiniLMForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = AVPrompt_MiniLM(config, multimodal_config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
