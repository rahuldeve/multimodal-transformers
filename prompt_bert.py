...
from globals import TEXT_DIM, ACOUSTIC_DIM, VISUAL_DIM, DEVICE
...
class MAGBert(BertModel):
    def __init__(self, config):
        ...
        self.fuse = True
        self.prompt_encoder = deepcopy(self.embeddings)
    ...
    def forward(
        ...
        # print(attention_mask.shape, input_ids.shape, visual.shape)
        ...
        # Early fusion with MAG
        if self.fuse:
            prompt_embedding_output = self.prompt_encoder(
                input_ids=input_ids,
                position_ids=position_ids,
                token_type_ids=token_type_ids,
                inputs_embeds=inputs_embeds,
            ...
            fused_embedding = self.mag(prompt_embedding_output, visual, acoustic)
            # print(fused_embedding.shape, attention_mask.shape)
            fused_embedding = torch.cat([fused_embedding, embedding_output], dim=1)
